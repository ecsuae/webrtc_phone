...
export function buildDiagFromSession(session, { aor } = {}) {
...
export function maybeEmitIceCompleteFromSdp(pc, label, dir, diag, { aor, callId, counts } = {}) {
  if (pc.iceGatheringState !== "complete") return;

  const sdpCounts = countCandidatesFromSdp(pc.localDescription?.sdp);
  try {
    if (counts && typeof counts === "object") Object.assign(counts, sdpCounts);
  } catch {}

  logLine(
    `[${nowISO()}] [pc:${label}] ICE already complete (from SDP) — host=${counts?.host ?? 0} srflx=${counts?.srflx ?? 0} relay=${counts?.relay ?? 0} total=${sdpCounts.total}`
  );
  if ((counts?.total ?? sdpCounts.total) === 0) {
