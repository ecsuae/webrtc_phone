export function setupDesktopCacheActions() {
  try {
    ...
    console.log(`[BOOT_MARKER_DESKTOP_CACHE_ACTIONS_SETUP] build=${build} cb=${cb} ts=${Date.now()}`);
  } catch {}
  ...
  window.clearAllCacheAndReload = async function () {
    ...
    try {
      try {
        const build = String(window.__APP_BUILD__ || "");
        const cb = String(window.__BUILD_CB || "");
        console.log(`[BOOT_MARKER_DESKTOP_CACHE_ACTIONS_RUN] build=${build} cb=${cb} ts=${Date.now()}`);
      } catch {}
